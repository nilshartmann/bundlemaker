...
import org.bundlemaker.core.ui.editor.dsm.widget.DsmViewContentReceiver;
...
public class VerticalSideMarker extends AbstractSideMarker {
	...
	private DsmViewContentReceiver dsmViewContentReceiver;
   ...
   * </p>
	 ...
	 * @param  colorScheme
	 */
	public VerticalSideMarker(
		IDsmContentProvider    contentProvider,
		ILabelProvider		   labelProvider,
		IDsmColorScheme		   colorScheme,
		DsmViewContentReceiver dsmViewContentReceiver
	) {
		super(contentProvider, labelProvider, colorScheme);

		this.dsmViewContentReceiver = dsmViewContentReceiver;
	}

	@Override
	protected void onMousePressed(MouseEvent me) {
		int verticalBoxSize = getBoxSize().getVerticalBoxSize();
		dragFrom = me.y / verticalBoxSize;
	...
	protected void onMouseReleased(MouseEvent me) {
		int verticalBoxSize = getBoxSize().getVerticalBoxSize();
		int dragTo = me.y / verticalBoxSize;
		String fromLabel = getLabelProvider().getText(getContentProvider().getNodes()[dragFrom]);
		String toLabel = getLabelProvider().getText(getContentProvider().getNodes()[dragTo]);
		System.out.println("mouseReleased move: " + fromLabel + " to: " + toLabel);

		getContentProvider().swap(dragFrom, dragTo);
		dsmViewContentReceiver.setModel(getContentProvider());
