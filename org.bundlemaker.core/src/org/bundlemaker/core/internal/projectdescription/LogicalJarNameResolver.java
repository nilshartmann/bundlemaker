...
public class LogicalJarNameResolver {
	...
	public final static String extractNameFromRootDirectory(JarFile jarFile) {
		...
		while (enumeration.hasMoreElements()) {
			...
			String directory = index != -1 ? jarEntry.getName().substring(0,
					index) : jarEntry.getName();
	...
	/**
	 * <p>
	 * Extracts the name from the input string. This methods first searches for
	 * a possible version string in the input name. If a version is found, the
	 * substring from 0 till the beginning of the version string will be
	 * returned.
	 * </p>
	 * 
	 * @param inputStr
