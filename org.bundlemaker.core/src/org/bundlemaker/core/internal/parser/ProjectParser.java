...
public class ProjectParser {
   ...
   *          the bundle maker project
   */
  public ProjectParser(BundleMakerProject bundleMakerProject, boolean parseIndirectReferences) {
   ...
   */
  public final List<IProblem> parseBundleMakerProject(IProgressMonitor progressMonitor) throws CoreException {

    // reset the store
    Activator.getDefault().getPersistentInfoStoreFactory().resetPersistentDependencyStore(_bundleMakerProject);
    ...
    // TODO
    progressMonitor.beginTask("parsing project: files to analyze ", computeArtifactCount());
    ...
    for (IFileBasedContent fileBasedContent : _bundleMakerProject.getProjectDescription().getFileBasedContent()) {

      progressMonitor.setTaskName(String.format("Parsing '%s'...", fileBasedContent.getName()));
  ...
  private void parseContent(IFileBasedContent content, IProgressMonitor progressMonitor, IResourceCache cache)
      throws CoreException {
    ...
    for (int i = 0; i < THREAD_COUNT; i++) {
      if ((i + 1) * partSize <= allDirectories.size()) {
        packageFragmentsParts[i] = allDirectories.subList(i * partSize, (i + 1) * partSize);
      } else if ((i) * partSize <= allDirectories.size()) {
        packageFragmentsParts[i] = allDirectories.subList(i * partSize, allDirectories.size());
      } else {
    ...
    for (int i = 0; i < _parsers.length; i++) {
      ...
      if (matches(parserType, content)) {
        ...
        ProgressMonitorWrapper monitorWrapper = new ProgressMonitorWrapper(progressMonitor);
        ...
        for (int threadIndex = 0; threadIndex < THREAD_COUNT; threadIndex++) {
          parserCallables[threadIndex] = new ParserCallable(content, packageFragmentsParts[threadIndex],
        ...
        for (int threadIndex = 0; threadIndex < THREAD_COUNT; threadIndex++) {
          futureTasks[threadIndex] = new FutureTask<List<IProblem>>(parserCallables[threadIndex]);
        ...
        int[] count = getResourcesToParseCount(content, parser4Thread[0]);
        int ticksToAdd = (count[0] + count[1]) - monitorWrapper.getWorked();
        if (ticksToAdd > 0) {
  ...
  private boolean matches(ParserType parserType, IFileBasedContent content) {
    return ((parserType.equals(ParserType.BINARY) || parserType.equals(ParserType.BINARY_AND_SOURCE)) && !content
        .getBinaryResources().isEmpty())
        || ((parserType.equals(ParserType.SOURCE) || parserType.equals(ParserType.BINARY_AND_SOURCE))
            && !content.getSourceResources().isEmpty() && content.isAnalyzeSourceResources());
  }
  ...
  private int[] countResourcesToParse(IFileBasedContent content) {
    ...
    if (content.isResourceContent()) {
      ...
      for (IParser[] parsers4Thread : _parsers) {
        ...
        int[] count = getResourcesToParseCount(content, parsers4Thread[0]);
   ...
   * @return
   */
  private int[] getResourcesToParseCount(IFileBasedContent content, IParser parser) {
    ...
    if (parser.getParserType().equals(ParserType.BINARY)) {
      ...
      return new int[] { content.getBinaryResources().size(), content.getSourceResources().size() };
    }
