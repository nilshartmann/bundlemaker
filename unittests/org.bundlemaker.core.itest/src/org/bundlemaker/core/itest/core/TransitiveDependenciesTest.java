...
public class TransitiveDependenciesTest extends AbstractModularizedSystemTest {
	...
	protected void moveType(String typeName, String targetModuleName) {
		...
		IDependencyModel dependencyModel = getDependencyModel();

		IArtifact root = dependencyModel.getRoot();
		Collection<IArtifact> leafs = root.getLeafs();

		for (IArtifact artifact : leafs) {
			if (typeName.equals(artifact
					.getQualifiedName())) {
				moveArtifact(artifact, targetModuleName, dependencyModel);
			}
		}
	}
	
	protected  IDependencyModel getDependencyModel() {
		IDependencyModel dependencyModel = ModelTransformer
		.getDependencyModel(
				(IModifiableModularizedSystem) getModularizedSystem(),
	...
	protected void addModule(String name, String... content) {
		ResourceSetBasedModuleDefinition moduleDefinition = _transformation
				.addModuleDefinition(name, "1.0.0");
		moduleDefinition.addResourceSet(new ModuleIdentifier(
				getTestProjectName(), "1.0.0"), content, new String[] {});
	}

	protected IResourceModule getResourceModule(String name) {
		IResourceModule resourceModule = getModularizedSystem()
				.getResourceModule(name, "1.0.0");
		assertNotNull(resourceModule);
		return resourceModule;
	}

	protected Set<IModule> getTransitiveReferencesModules(String moduleName) {
		Set<IModule> referencedModules = getModularizedSystem()
		.getTransitiveReferencedModules(
				getResourceModule(moduleName))
		.getReferencedModules();
		assertNotNull(referencedModules);
		return referencedModules;
	}

	private void moveArtifact(final IArtifact typeArtifact, String moduleName,
			final IDependencyModel model) {

		// das root object
		IRootArtifact rootArtifact = (IRootArtifact) model.getRoot();

		//
		IModuleArtifact moduleArtifact = rootArtifact.getOrCreateModule(
				moduleName, "1.0.0");
