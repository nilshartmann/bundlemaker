...
public class DefaultManifestCreator extends AbstractManifestCreator {
	...
	/**
	 * <p>
	 * Determines if the specified package should be imported using
	 * Import-Package.
	 * </p>
	 * 
	 * @param packageName
	 ...
	 * @return true if the package should be imported or false if a
	 *         Require-Bundle header should be added to the Manifest
	 */
	protected boolean useImportPackage(String packageName,
			List<IModuleArtifact> exportingModules) {
		...
		default:
			throw new IllegalArgumentException(
					"Unknown DependencyStyle enum value '"
	...
	/**
	 * <p>Determine if the reference to the given package should be skipped, i.e. if
	 * it should <b>not</b> be added to manifest via Import-Package or Require-Bundle at all.
	 * </p>
	 * 
	 * <p>This implementation does <b>not</b> skip any references but keeps them all. Override
	 * this method to provide different behavior, for example to skip references to packages
	 * from the boot classloader (e.g. sun.*)
	 *
	 * @param packageName The package name that is required by the bundle
	 * @param value modules that the required package provide
	 * @return true to skip the reference (don't add it to the manifest) or true to include it
	 * via Import-Package or Require-Bundle)
	 */
	protected boolean skipReferencedPackage(String packageName,
			List<IModuleArtifact> exportingModules) {
		return false;
	}
	...
	/**
	 * <p>
	 * Sets the bundle's symbolic name to the name of the associated
	 * {@link IResourceModule}.
	 * </p>
	 */
	protected void setBundleSymbolicName() {
		getBundleManifest().getBundleSymbolicName().setSymbolicName(
				getResourceModule().getModuleIdentifier().getName());
	}

	/**
	 * <p>
	 * Sets the bundle's name to the name of the associated
	 * {@link IResourceModule}.
	 * </p>
	 */
	protected void setBundleName() {
		getBundleManifest().setBundleName(
				getResourceModule().getModuleIdentifier().getName());
	}

	/**
	 * <p>
	 * Sets the bundle's version to the version of the associated
	 * {@link IResourceModule}, or '0.0.0' if the module's version is not a
	 * valid OSGi version.
	 * </p>
	 */
	protected void setBundleVersion() {
		if (ManifestUtils.isValidOSGiVersion(getResourceModule()
				.getModuleIdentifier().getVersion())) {
			getBundleManifest().setBundleVersion(
					new Version(getResourceModule().getModuleIdentifier()
							.getVersion()));
		} else {
	...
	/**
	 * <p>
	 * Sets the fragment host for the bundle if the associated module is tagged
	 * as a fragment module.
	 * </p>
	 */
	protected void setFragmentHost() {
		if (ManifestUtils.isFragment(getResourceModule())) {
			IModule hostModule = ManifestUtils
					.getFragmentHost(getResourceModule());
			if (hostModule != null) {
				getBundleManifest().getFragmentHost().setBundleSymbolicName(
						hostModule.getModuleIdentifier().getName());
			} else {
				// TODO
			}
		}
	...
	/**
	 * <p>
	 * Copies all headers from the original manifest except the headers listed
	 * in {@link IManifestConstants#ORIGINAL_HEADERS_NOT_TO_COPY}.
	 * </p>
	 */
	protected void copyOriginalManifestHeader() {
		for (String key : ManifestUtils.getHeaders(getOriginalManifest())) {
			if (!containsIgnoreCase(key,
					IManifestConstants.ORIGINAL_HEADERS_NOT_TO_COPY)) {
				getBundleManifest().setHeader(key,
						getOriginalManifest().getHeader(key));
			}
	 ...
	 * <p>
	 * Copies all headers from the original manifest except the headers listed
	 * in {@link IManifestConstants#TEMPLATE_HEADERS_NOT_TO_COPY}.
	 * </p>
	 */
	protected void copyTemplateManifestHeader() {
		for (String key : ManifestUtils.getHeaders(getManifestTemplate())) {
			if (!containsIgnoreCase(key,
					IManifestConstants.TEMPLATE_HEADERS_NOT_TO_COPY)) {
				getBundleManifest().setHeader(key,
						getManifestTemplate().getHeader(key));
			}
	...
	protected void setExportPackage() {
		...
		for (IPackageArtifact packageArtifact : ArtifactUtils
				.getAllChildPackages(getModuleArtifact())) {

			// only export packages that contain types - do NOT export the
			// default package
			if (packageArtifact.containsTypes()
					&& packageArtifact.getQualifiedName().trim().length() > 0) {

				//
				if (hasMostSpecificPackageTemplate(
						IManifestConstants.HEADER_EXCLUDED_EXPORTS,
						packageArtifact.getQualifiedName())) {
					continue;
				}

				//
				ExportedPackage exportedPackage = getBundleManifest()
						.getExportPackage().addExportedPackage(
								packageArtifact.getQualifiedName());

				// RULE: set export package version
				if (exportedPackage.getVersion().toString() != getResourceModule()
						.getModuleIdentifier().getVersion()
						&& !getManifestPreferences().noExportPackageVersion()
						&& ManifestUtils.isValidOSGiVersion(getResourceModule()
								.getModuleIdentifier().getVersion())) {

					//
					exportedPackage.setVersion(new Version(getResourceModule()
							.getModuleIdentifier().getVersion()));
				}

				// rule "set attributes and directives from template manifest"
				if (hasMostSpecificPackageTemplate(
						IManifestConstants.HEADER_EXPORT_TEMPLATE,
						exportedPackage.getPackageName())) {

					HeaderDeclaration header = getMostSpecificPackageTemplate(
							IManifestConstants.HEADER_EXPORT_TEMPLATE,
							exportedPackage.getPackageName());
	...
	protected void setImportPackageAndRequireBundle() {
		...
		Collections.sort(entrySet,
				new Comparator<Entry<String, List<IModuleArtifact>>>() {

					@Override
					public int compare(
							Entry<String, List<IModuleArtifact>> arg0,
							Entry<String, List<IModuleArtifact>> arg1) {
		...
		for (Entry<String, List<IModuleArtifact>> packageEntry : entrySet) {
			...
			if (useImportPackage(packageEntry.getKey(), packageEntry.getValue())) {
				...
				//
				importWithImportPackage(packageEntry.getKey(),
			...
			// 'Require-Bundle' instead
				...
				//
				importWithRequiredBundle(packageEntry.getKey(),
						packageEntry.getValue(), requiredBundles);
			}
		}

		//
		for (ImportedPackage importedPackage : getManifestTemplate()
				.getImportPackage().getImportedPackages()) {
			if (!containsImportedPackage(importedPackage.getPackageName())) {
				getBundleManifest().getImportPackage().getImportedPackages()
						.add(importedPackage);
			}
		}

		//
		for (RequiredBundle requiredBundle : getManifestTemplate()
				.getRequireBundle().getRequiredBundles()) {
			if (!containsBundle(requiredBundle.getBundleSymbolicName())) {
				getBundleManifest().getRequireBundle().getRequiredBundles()
						.add(requiredBundle);
			}
		}
	}
	 ...
	 * @param packageName
	 */
	protected void importWithImportPackage(String packageName,
			List<IModuleArtifact> moduleArtifacts) {
		...
		if (moduleArtifacts.size() > 1) {
			...
			System.out.println();
			System.out
					.println(" Split-packages can not be imported with Import-Package! "
							+ packageName + " : " + moduleArtifacts);
			System.out.println();
			System.out.println(" ********************************** ");
		}
		...
		for (IModuleArtifact moduleArtifact : moduleArtifacts) {
			...
			if (moduleArtifact.getAssociatedModule() != null
					&& (moduleArtifact.getAssociatedModule().equals(
							getResourceModule()) || ManifestUtils
							.isHostForResourceModule(
									moduleArtifact.getAssociatedModule(),
									getResourceModule()))) {

				// TODO: Error?
				System.out.println(" **** filter self-hosted packages: "
						+ packageName + " : " + moduleArtifacts);
				return;
			}
		}
		...
		if (hasMostSpecificPackageTemplate(
				IManifestConstants.HEADER_EXCLUDED_IMPORTS, packageName)) {
			return;
		}

		// Rule: import the required package
		ImportedPackage importedPackage = getBundleManifest()
				.getImportPackage().addImportedPackage(packageName);
		...
		if (moduleArtifact.getAssociatedModule() != null) {
			...
			if (duplicatePackageProvider != null) {
				...
				for (IPackageArtifact iPackageArtifact : duplicatePackageProvider) {
					IArtifact exportingModuleArtifact = iPackageArtifact
							.getParent(ArtifactType.Module);

					IModule associatedModule = ((IModuleArtifact) exportingModuleArtifact)
							.getAssociatedModule();
					...
					if (associatedModule == null
							|| !associatedModule
									.getUserAttributes()
									.containsKey(
											IManifestConstants.OSGI_FRAGMENT_HOST)) {
						hostModules++;
					}
				}
				...
				if (hostModules > 1) {
					...
					// get the module name
					String moduleName = getModularizedSystem()
							.getExecutionEnvironment().equals(module) ? Constants.SYSTEM_BUNDLE_SYMBOLICNAME
		...
		HeaderDeclaration headerDeclaration = getMostSpecificPackageTemplate(
				IManifestConstants.HEADER_IMPORT_TEMPLATE, packageName);
		if (headerDeclaration != null) {
			importedPackage.getAttributes().putAll(
					headerDeclaration.getAttributes());
			importedPackage.getDirectives().putAll(
					headerDeclaration.getDirectives());
		}
	}
	...
	protected Collection<IPackageArtifact> getDuplicatePackageProvider(
		...
		return _duplicatePackagesVisitor
				.getDuplicatePackageProvider(packageName);
	}
	 ...
	 */
	protected void importWithRequiredBundle(String packageName,
			List<IModuleArtifact> modules, List<IModule> requiredBundles) {
		...
		for (IModuleArtifact exportingModuleArtifact : modules) {
			...
			// get the module
			IModule exportingModule = exportingModuleArtifact
					.getAssociatedModule();

			// rule: filter 'self-requires'
			if (exportingModule.equals(getModuleArtifact()
					.getAssociatedModule())) {
				continue;
			}

			// get the host module
			IModule exportingHostModule = ManifestUtils
					.getFragmentHost(exportingModule);

			// filter 'self-requires'
			if (exportingHostModule.equals(getModuleArtifact()
					.getAssociatedModule())) {
				continue;
			}
			...
			if (ManifestUtils.isHostForResourceModule(
					exportingModuleArtifact.getAssociatedModule(),
					getResourceModule())) {
				continue;
			}

			// create a required bundle
			String symbolicName = getModularizedSystem()
					.getExecutionEnvironment().equals(exportingHostModule) ? Constants.SYSTEM_BUNDLE_SYMBOLICNAME
					: exportingHostModule.getModuleIdentifier().getName();

			//
			RequiredBundle requiredBundle = getBundleManifest()
					.getRequireBundle().addRequiredBundle(symbolicName);

			// set a version range if multiple versions exist
			if (getModularizedSystem().getModules(
					exportingHostModule.getModuleIdentifier().getName()).size() > 1) {

				requiredBundle.setBundleVersion(new VersionRange("["
						+ exportingHostModule.getModuleIdentifier()
								.getVersion()
						+ ","
						+ exportingHostModule.getModuleIdentifier()
								.getVersion() + "]"));
			...
			// Rule: add templates attributes and parameter
			HeaderDeclaration headerDeclaration = getMostSpecificPackageTemplate(
					IManifestConstants.HEADER_REQUIRE_BUNDLE_TEMPLATE,
					exportingHostModule.getModuleIdentifier().getName());
			if (headerDeclaration != null) {
	...
	protected void setTransitiveClosure() {
		...
		//
		for (IModule module : getModularizedSystem()
				.getTransitiveReferencedModules(getResourceModule())
				.getReferencedModules()) {

			//
			if (!containsBundle(module.getModuleIdentifier().getName())
					&& !module.equals(getModularizedSystem()
		...
		if (!transitiveClosure.isEmpty()) {
			...
			for (Iterator<String> iterator = transitiveClosure.iterator(); iterator
					.hasNext();) {
			...
			getBundleManifest().setHeader(
					IManifestConstants.TRANSITIVE_CLOSURE,
	...
	protected final GenericCache<String, List<IModuleArtifact>> createPackageToExportingModulesCache() {
		...
		for (IDependency dependency : getModuleArtifact().getDependencies()) {
			...
			// TODO
			IBundleMakerArtifact bundleMakerArtifact = (IBundleMakerArtifact) dependency
					.getTo();

			// TODO
			IPackageArtifact packageArtifact = (IPackageArtifact) bundleMakerArtifact
					.getParent(ArtifactType.Package);
			IModuleArtifact moduleArtifact = (IModuleArtifact) packageArtifact
					.getParent(ArtifactType.Module);

			//
			List<IModuleArtifact> modules = requiredPackages
					.getOrCreate(packageArtifact.getQualifiedName());
	 ...
	 * @return
	 */
	public boolean hasMostSpecificPackageTemplate(String headerKey,
			String packageName) {
	 ...
	 * @return
	 */
	protected HeaderDeclaration getMostSpecificPackageTemplate(
			String headerKey, String packageName) {

		// get the import package template
		String templateHeader = getManifestTemplate().getHeader(headerKey);

		//
		List<HeaderDeclaration> packageTemplates = ManifestUtils
				.parseManifestValue(templateHeader);

		// get the template
		return ManifestUtils.findMostSpecificDeclaration(packageTemplates,
				packageName);
	}
	...
	protected boolean isExcludedRequiredBundle(IModule module) {
		...
		//
		List<HeaderDeclaration> excludedRequiredBundleTemplates = ManifestUtils
				.parseManifestValue(templateHeader);

		// get the template
		HeaderDeclaration declaration = ManifestUtils
				.findMostSpecificDeclaration(excludedRequiredBundleTemplates,
						module.getModuleIdentifier().getName());
	...
	private boolean containsBundle(String requiredBundleName) {
		...
		//
		for (RequiredBundle alreadyRequiredBundle : getBundleManifest()
				.getRequireBundle().getRequiredBundles()) {

			//
			if (alreadyRequiredBundle.getBundleSymbolicName().equalsIgnoreCase(
					requiredBundleName)) {
				return true;
			}
		}
	...
	private boolean containsImportedPackage(String importedPackageName) {
		...
		//
		for (ImportedPackage alreadyImportedPackage : getBundleManifest()
				.getImportPackage().getImportedPackages()) {

			//
			if (alreadyImportedPackage.getPackageName().equalsIgnoreCase(
					importedPackageName)) {
				return true;
			}
		}
